      ...
      {navItems.map((item) => {
        ...
        if (item.isCenter) {
          return (
            ...
            >
              <div className="w-14 h-14 rounded-full bg-gradient-to-r from-indigo-600 via-blue-600 to-indigo-700 text-white flex items-center justify-center shadow-lg shadow-indigo-500/30 border-4 border-slate-50 dark:border-slate-950 active:scale-95 group-hover:scale-105 transition-all">
                {item.icon}
              </div>
              <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 mt-0.5 tracking-wide">
                {item.label}
              </span>
            </button>
          );
        ...
        return (
          ...
          >
            <div className={`p-1 rounded-lg ${isActive ? "bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400" : ""}`}>
              {item.icon}
            </div>
